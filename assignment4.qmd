...
```{sql connection= con, message = FALSE}
SELECT EXTRACT(HOUR FROM date) as hour_of_day, count(*)
FROM crime
WHERE YEAR = 2016
GROUP BY date 
...
All of the top 10 above appear to all be from hour 12, meaning noon. So noon is the hour of the day associated with the most arrests in the Chicago database.
...
# Results look good, we can see BATTERY with 3906 arrests and the other crimes in descending order.
```
...
# It appears arranging by year starts off at 2001 by default.
```
